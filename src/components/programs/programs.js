                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                            {(loading ? Array(8).fill({}) : programs).map((program, index) => (
                                <div 
                                    ...
                                    className={`group relative bg-white dark:bg-slate-950 border border-slate-200 dark:border-primary/10 overflow-hidden rounded-[2rem] shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-1000 transform ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-[50px] opacity-0'} ${loading ? 'animate-pulse' : ''}`}
                                    style={{ 

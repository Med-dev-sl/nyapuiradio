    ...
    const fetchPrograms = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter out inactive programs and take only the first 4
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-l border-t border-slate-200 dark:border-primary/20">
        {programs.map((program, index) => (
          <div 
            ...
            className={`group relative bg-white dark:bg-slate-950 border-r border-b border-slate-200 dark:border-primary/10 overflow-hidden rounded-none transition-all duration-1000 transform ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-[200px] opacity-0'}`}
            style={{ 
          ...
          >
            ...
            <div className="p-8 space-y-6">
              ...
              {/* Time Display - Industrial/Rectangular Style */}
            ...
            <div className="absolute bottom-1 right-1 opacity-0 group-hover:opacity-10 scale-0 group-hover:scale-100 transition-all duration-700">
                <span className="material-symbols-outlined text-8xl -rotate-12 translate-x-8 translate-y-8">radio_button_checked</span>
            </div>
          </div>
        ))}
      </div>
      
      {/* Dynamic Slide-in Background Text */}
      <div className={`absolute -right-20 bottom-10 text-9xl font-black text-slate-100 dark:text-white/[0.02] pointer-events-none transition-all duration-[2000ms] ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-40 opacity-0'}`}>
        PROGRAMS
      </div>
    </section>
  );

...
async function initializeSchema() {
  ...
  await runSql(`CREATE TABLE IF NOT EXISTS news (id ${idType}, title ${textType} NOT NULL, category ${textType} NOT NULL, headline ${textType} NOT NULL, details ${textType}, caption ${textType}, photo ${textType}, thumbnail ${textType}, status ${textType} DEFAULT 'Draft', created_at ${timestampType});`);
  await runSql(`CREATE TABLE IF NOT EXISTS podcasts (id ${idType}, title ${textType} NOT NULL, description ${textType}, thumbnail ${textType}, video_url ${textType}, status ${textType} DEFAULT 'Draft', created_at ${timestampType});`);
  await runSql(`CREATE TABLE IF NOT EXISTS program_videos (id ${idType}, program_id INTEGER NOT NULL, title ${textType} NOT NULL, description ${textType}, thumbnail ${textType}, video_url ${textType}, status ${textType} DEFAULT 'Draft', created_at ${timestampType} ${isPostgres ? ', CONSTRAINT fk_program FOREIGN KEY (program_id) REFERENCES programs(id) ON DELETE CASCADE' : ', FOREIGN KEY (program_id) REFERENCES programs(id) ON DELETE CASCADE'});`);
